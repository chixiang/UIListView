...
@class UZASIDataDecompressor;

extern NSString *kUZASIHTTPRequestVersion;
...
extern NSString* const kUZNetworkRequestErrorDomain;
...
extern unsigned long const kUZASIWWANBandwidthThrottleAmount;
...
@interface UZASIHTTPRequest : NSOperation <NSCopying> {
	...
	id <ASIHTTPRequestDelegate, UZASIProgressDelegate> queue;
	...
	id <UZASIProgressDelegate> uploadProgressDelegate;
	...
	id <UZASIProgressDelegate> downloadProgressDelegate;
	...
	id <UZASICacheDelegate> downloadCache;
	...
	UZASIDataDecompressor *dataDecompressor;
	...
	UZASIHTTPRequest *PACFileRequest;
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UZASICacheDelegate>)cache;
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <UZASICacheDelegate>)cache andCachePolicy:(ASICachePolicy)policy;
...
- (NSData*)getUserResponseData:(NSData*)data;
...
+ (void)setDefaultCache:(id <UZASICacheDelegate>)cache;
+ (id <UZASICacheDelegate>)defaultCache;
...
+ (NSThread *)threadForRequest:(UZASIHTTPRequest *)request;
...
@property (assign) UZASIHTTPRequest *mainRequest;
...
@property (assign) id <UZASICacheDelegate> downloadCache;
...
#endif
@property (retain) UZASIDataDecompressor *dataDecompressor;
